...
public class ClienteDAO extends CustomConnection {
    ...
    /**
     * Guarda un usuario con los datos (Nombre, Apellido Paterno, Apellido Materno, Correo, Alcaldia, CP, Calle, Num Exterior, Num Interior, Id del Estado, Id de Escolaridad).
    ...
    public String saveCliente(Object[] cliente) {
        ...
        try {
            ...
            if(resultSet.next()) {
               clienteId = Integer.toString(resultSet.getInt(1) + 1);
            } 
            ...
            int cred_id = credDAO.saveCredencial(next_year);
            
            // Resto del código para insertar el usuario en CLIENTE
            sentenciaSQL = "INSERT INTO CLIENTE VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            preparedStatement = connection.prepareStatement(sentenciaSQL);

            preparedStatement.setBigDecimal(1, new BigDecimal(clienteId));                 // Id (calculado).
            ...
            preparedStatement.setBigDecimal(8, new BigDecimal(cred_id));                   // Id de credencial.
